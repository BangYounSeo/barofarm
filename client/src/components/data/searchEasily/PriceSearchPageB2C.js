//client/src/components/data/searchEasily/PriceSearchPageB2C.js
...
import zucchini from '../../../assets/items/zucchini.png';
import aehobak from '../../../assets/items/aehobak.png';
import mustardGreens from '../../../assets/items/mustardGreens.png';
...
export const itemImages = {
  ...
  "호박/쥬키니": zucchini,
  "갓/갓": mustardGreens,  
  "고춧가루/국산" : pepperPowder,
  "고춧가루/중국" : pepperPowder,  
  "고구마/밤":sweetpotato,
  "건고추/화건":drypepper,
  "깻잎/깻잎":perillaleaf,
...
const ITEM_LIST_API =
  "/api/easy-price/items/distinct/kind01";

const today = dayjs().format("YYYY-MM-DD");
const yesterday = dayjs().subtract(1, "day").format("YYYY-MM-DD");

export default function PriceSearchPageB2C() {
  const firstLoad = useRef(true)
  const {state} = useLocation(); //메인페이지에서 그래프클릭했을떄 쓰려고

  const savedItem = localStorage.getItem("savedItem_b2c") || "감귤/노지";
  const savedRegday = localStorage.getItem("savedRegday_b2c") || yesterday;

  const [itemName, setItemName] = useState(
    state?.itemName || savedItem
  );
  const [regday, setRegday] = useState(
    state?.regday || savedRegday  
  );
  const clsCode = state?.clsCode || "01";
  ...
  useEffect(() => {
    if (itemName) {
      localStorage.setItem("savedItem_b2c", itemName);
