...
const color = theme.palette.primary.main;
  ...
  const fetchYearlySales = (target) => {
    ...
    .then(res => {
      const itemsByCode = res.data.filter(entry => entry.productclscode === "01");
...
if (maxMinRegion.length > 0) {
  ...
  return (
    <Container>
      ...
      <Typography variant="h5" sx={{ fontWeight: 400 ,pt:2}} >
      ...
      <Typography variant="h5" gutterBottom sx={{ fontWeight: 600 ,pt:7}}>
